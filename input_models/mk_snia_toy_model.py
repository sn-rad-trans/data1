...
22 Jun 2018 - revised version (SB)
              o revised defaults to match high-56Ni-mass toy model
...
### defaults
MTOT_INIT = 1.0              # total mass (msun)
EKIN_INIT = 1.0              # asymptotic kinetic energy (1e51 erg)
DVEL_INIT = 50.0             # cell size (km/s)
...
MIGE_INIT = 0.0              # stable IGE mass (msun)
MNI56_INIT = 0.6             # 56Ni mass at t=0 (msun)
MIME_INIT = 0.4              # IME mass (msun)
DMIGE_INIT = 0.1             # mass interval over which stable IGE mass fraction transitions from 1 to 0 (msun)
DMNI56_INIT = 0.4            # mass interval over which 56Ni mass fraction transitions from 1 to 0 (msun)
...
TRANSPROF_INIT = 'cosine'    # transition profile for mass fraction variation from 1 to 0: 'linear', 'invexpon' (inverse exponential) or 'cosine' (cosine bell)
...
T_END = 2.0                  # final time for toy model (days)
TEMP_MIN = 5e3               # minimum allowed temperature (K)
...
if __name__ == '__main__':
    ...
    # options
    #
    parser.add_argument('--highni', action='store_true', help='sets defaults for high-56Ni-mass toy model')
    parser.add_argument('--lowni', action='store_true', help='sets defaults for low-56Ni-mass toy model')
    ...
    parser.add_argument('--logx', action='store_true', help='plot log(X) for mass fractions')
    ...
    #
    # set defaults for --highni and --lowni cases
    #
    if args.highni or args.lowni:
        args.mtot = 1.0
        args.ekin = 1.0
        args.dvel = 50.0
        args.densprof = 'expon'
        ...
        args.mige = 0.0
        args.dmfine = 1e-4
        args.transprof = 'cosine'
        args.xfracti = 0.0
        args.ime = 'ca,s,si'
        args.xfracime = '0.1,0.35,0.55'
        if args.highni:
            args.mni56 = 0.6
            args.dmni56 = 0.4
            args.mime = 0.4
            args.fout = 'snia_toy06.dat'
        elif args.lowni:
            args.mni56 = 0.1
            args.dmni56 = 0.2
            args.mime = 0.9
            args.fout = 'snia_toy01.dat'
    ...
    # time-weighted integral of deposition from Ni (per Ni nucleus at t=0)
    qtdtni = inttexpni / tauni * EDECAY_56NI * MEV2ERG
    
    # time-weighted integral of deposition from Co (per Ni nucleus at t=0)
    ...
    print('')
    print('  qtdt_per_nucleus = {:.4e} erg s'.format(qtdt))
    ...
    if not args.noplot:
        ...
        ax.set_xlim(0., vel.max()/1e3)
        if args.logx:
            ax.set_ylabel('Log$_{10}$ Mass Fraction $X_i$')
            ax.set_ylim(-5,0.95)
            if mige > EPSILON:
                idx = np.where(xige > 0.0)[0]
                ax.plot(vel[idx]/1e3, np.log10(xige[idx]), marker='.', label='stable IGE')
            idx = np.where(xni56 > 0.0)[0]
            ax.plot(vel[idx]/1e3, np.log10(xni56[idx]), marker='.', label='$^{56}$Ni')
            idx = np.where(xime > 0.0)[0]
            ax.plot(vel[idx]/1e3, np.log10(xime[idx]), marker='.', label='IME')
            if munbco > EPSILON:
                idx = np.where(xunbco > 0.0)[0]
                ax.plot(vel[idx]/1e3, np.log10(xunbco[idx]), marker='.', label='unburnt C/O')
        else:
        ...
        if args.logx:
            ...
            if mige > EPSILON:
                idx = np.where(xige > 0.0)[0]
                ax.plot(mass[idx], np.log10(xige[idx]), marker='.', label='stable IGE')
            idx = np.where(xni56 > 0.0)[0]
            ax.plot(mass[idx], np.log10(xni56[idx]), marker='.', label='$^{56}$Ni')
            idx = np.where(xime > 0.0)[0]
            ax.plot(mass[idx], np.log10(xime[idx]), marker='.', label='IME')
            if munbco > EPSILON:
                idx = np.where(xunbco > 0.0)[0]
                ax.plot(mass[idx], np.log10(xunbco[idx]), marker='.', label='unburnt C/O')
        else:
            ax.set_ylabel('Mass Fraction $X_i$')
            ax.set_ylim(-.05,1.05)
            if mige > EPSILON:
        ...
        # mass profile in separate figure
